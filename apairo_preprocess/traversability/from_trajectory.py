...
class TraversabilityFromTrajectory(FramePreprocessor):
    ...
    Args:
        ...
        sequence_gap:   Distance threshold (metres) to detect sequence boundaries
                        and avoid look-ahead across discontinuous sessions.
    """
    ...
    def __init__(
        ...
        boundaries = np.where(dists > sequence_gap)[0]
    ...
    def process(self, sample: Sample) -> np.ndarray:
        ...
        self._idx += 1
        seq_end = int(self._seq_end[self._idx - 1]) + 1
        end = min(
            self._idx + self._forward_window if self._forward_window is not None else seq_end,
            seq_end,
        )
        future_pos = self._poses[self._idx : end, :3, 3]
